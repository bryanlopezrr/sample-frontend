...
function closeModal(modal_container: HTMLElement) {
...
function openModal(modal_container: HTMLElement) {
    modal_container.style.opacity = '1'
    modal_container.style.pointerEvents = 'auto'
}

class Login {
    constructor(public form: HTMLFormElement, public fields: Array<string>) {
        ...
        this.form.addEventListener("submit", (e) => {
            ...
            instance.fields.forEach((field) => {
                ...
                if (error == 0 ) {
                    ...
                    //modal function here
                    const modal_container = document.getElementById('modal-container') as HTMLElement;
                    ...
                    const credentials = document.getElementById('loginForm') as HTMLFormElement
                    if((credentials?.elements as any).userinput.value == "username" && 
                    (credentials?.elements as any).passinput.value == "password") 
                    {
    ...
    validateFields(field: HTMLInputElement) {
        ...
        if (field.value.trim() === "") {
            ...
            this.setStatus(
                field,
                `${(field.previousElementSibling as HTMLElement).innerText} Password cannot be blank`,
                "error"
            );
            return false;
        } else {
            ...
            if (field.type == "password") {
                ...
                if (field.value.length < 8) {
                    ...
                    this.setStatus(
                        field,
                        `${(field.previousElementSibling as HTMLElement).innerText} Password must be at least 8 characters`,
                        "error"
                    );
                    return false;
                } else {
    ...
    setStatus(field: HTMLInputElement, message: string | null, status: "success" | "error") {
